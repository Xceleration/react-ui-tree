...
export default class UITreeNode extends Component {
  ...
  renderCollapse = () => {
    ...
    if (index.children && index.children.length) {
      const { collapsed, draggable } = index.node;

      return (
        <span
          className={cx('collapse', collapsed ? 'caret-right' : 'caret-down', draggable === false ? 'disabled' : '')}
  ...
  render() {
    ...
    return (
      ...
      >
        <div
          className={cx('inner', node.draggable === false ? 'disabled' : '')}
          ref={this.innerRef}
